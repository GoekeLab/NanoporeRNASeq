...
overall estimated coverage distribution
```{r, fig.width = 12}
library(ggbio)
...
bamfile <- BamFile(bamfile_path)
ga <- readGAlignments(bamfile, use.names = TRUE)
autoplot(ga, method="estimate", xlab="coordinate", ylab="estimated coverage")
```
coverage of a single gene ENST00000215832 (MAPK1)
```{r, fig.width = 12}
range <- annotation$ENST00000215832
autoplot(bamfile,  method="raw", which=range)
```
