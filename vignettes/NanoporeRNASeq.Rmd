...
Option 1: get the path to the genome fasta file from the package
```{r}
...
bamfile_name <- bamFileNames[1]
bamfile_path <- readlist[1]
#example: the coverage of MAPK1 (chr22:21759657-21867680)
range <- GRanges("22",IRanges(21759657,21867680))
autoplot(bamfile_path,which=range)
```
...
###Run bambu
Option 1: apply *bambu* to perform EM on  extended annotations
```{r}
seExtended <- bambu(reads = readlist, annotations = annotationGrangesList_chr22_1_25409234, genomeSequence = "BSgenome.Hsapiens.NCBI.GRCh38", extendAnnotations = TRUE, verbose = FALSE, ncore = 6)
seExtended
```
Option 2: apply *bambu* for quantification without isoform discovery
```{r}
se <- bambu(reads = readlist, annotations = annotationGrangesList_chr22_1_25409234, genomeSequence = "BSgenome.Hsapiens.NCBI.GRCh38", extendAnnotations = FALSE, verbose = FALSE, ncore = 6)
se
```
```{r}
